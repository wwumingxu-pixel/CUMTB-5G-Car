...
public:
    ...
    const int binary_thresh_block_size = 11; // 二值化块大小（奇数）
    const int binary_thresh_C = 2;         // 二值化常数
    // 斑马线检测核心参数（类内初始化，仅一次）
...
// 斑马线检测函数
int detect_sidewalk(const Mat& frame) {
    // 提取固定行
    Mat y_line = frame.row(sidewalk_detect_y);

    // 转换为灰度图像（如果输入是彩色图像）
    Mat gray_line;
    if (y_line.channels() == 3) {
        cvtColor(y_line, gray_line, COLOR_BGR2GRAY);
    } else {
        gray_line = y_line.clone();
    }

    // 二值化处理（自适应阈值）
    Mat binary_line;
    adaptiveThreshold(gray_line, binary_line, 255, ADAPTIVE_THRESH_MEAN_C, THRESH_BINARY, binary_thresh_block_size, binary_thresh_C);

    // 可选：使用普通二值化（固定阈值）
    // threshold(gray_line, binary_line, 128, 255, THRESH_BINARY);
    ...
    // 从左到右查找第一个非零像素（左边界线）
    for (int x = 0; x < binary_line.cols; x++) {
        if (binary_line.at<uchar>(0, x) > 0) {
            left_line_x = x;
            break;
        }
    }
    ...
    // 从右到左查找第一个非零像素（右边界线）
    for (int x = binary_line.cols - 1; x >= 0; x--) {
        if (binary_line.at<uchar>(0, x) > 0) {
            right_line_x = x;
            break;
        }
    ...
    if (left_line_x != -1 && right_line_x != -1) {
        int white_count = right_line_x - left_line_x; // 左右边线之间的距离
        cout << "Left Line: " << left_line_x << ", Right Line: " << right_line_x << ", Width: " << white_count << endl;

        // 如果左右边界线距离小于阈值，检测到斑马线
        if (white_count < sidewalk_len) {
            cout << "Sidewalk detected!" << endl;

            // 模拟停车操作（例如控制电机速度为低速）
            // gpioPWM(motor_pin, 128); // 停车或低速行驶
...
int main(){
    ...
    int AB_stop_time = 0;
    // 电机速度参数（展示图像时车停止）
    int motor_speed = 204;// 191 -206，原210
    ...
    while (true) {
        ...
        // 调整图像大小（可选，根据实际摄像头分辨率调整）
        Mat resized_frame;
        resize(frame, resized_frame, Size(320, 180));

        // 检测斑马线
        int sidewalk_detected = detect_sidewalk(resized_frame);

        // 显示结果
        if (sidewalk_detected) {
            putText(resized_frame, "Sidewalk Detected!", Point(10, 30), FONT_HERSHEY_SIMPLEX, 0.7, Scalar(0, 0, 255), 2);
        } else {
            putText(resized_frame, "No Sidewalk", Point(10, 30), FONT_HERSHEY_SIMPLEX, 0.7, Scalar(0, 255, 0), 2);
        }

        // 显示图像
        imshow("Sidewalk Detection", resized_frame);
